...
void FFT_external_benchmark(float2 *d_input, float2 *d_output, int nSamples, int nSpectra, double *FFT_time, int inverse){
	GpuTimer timer;
...
void FFT_multiple_benchmark(float2 *d_input, float2 *d_output, int nSamples, int nSpectra, double *FFT_time, int inverse){
	GpuTimer timer;
	//---------> CUDA block and CUDA grid parameters
...
int GPU_FFT(float2 *h_input, float2 *h_output, int nSamples, int nSpectra, int inverse){
	...
	if (DEBUG) printf("Multiple FFT...: \t\t\t");
		FFT_init();
		FFT_multiple_benchmark(d_input, d_output, nSamples, nSpectra, &FFT_multiple_time, inverse);
		if (DEBUG) printf("done in %g ms.\n", FFT_multiple_time);
	}
	...
	if (DEBUG) printf("FFT...: \t\t\t\t");
		FFT_init();
		FFT_external_benchmark(d_input, d_output, nSamples, nSpectra, &FFT_external_time, inverse);
		if (DEBUG) printf("done in %g ms.\n", FFT_external_time);
	}
	...
	if (WRITE){ 
		char str[200];
		sprintf(str,"GPU-FFT-Pease_no_reorder.dat");

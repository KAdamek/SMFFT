...
__global__ void FFT_GPU_external(float2 *d_input, float2* d_output) {
	...
	#pragma unroll
	for(int f=0; f<8; f++){
		s_input[threadIdx.x + f*(FFT_LENGTH/8)]=d_input[threadIdx.x + f*(FFT_LENGTH/8) + blockIdx.x*FFT_LENGTH];
	}
	...
	#pragma unroll
	for(int f=0; f<8; f++){
		d_output[threadIdx.x + f*(FFT_LENGTH/8) + blockIdx.x*FFT_LENGTH]=s_input[threadIdx.x + f*(FFT_LENGTH/8)];
	}
}

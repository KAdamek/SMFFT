...
__global__ void FFT_GPU_external(float2 *d_input, float2* d_output, int N, int bits) {
	...
	for(int f=0; f<4; f++){
		s_input[threadIdx.x + f*(N/4)]=d_input[threadIdx.x + f*(N/4) + blockIdx.x*N];
	}
	...
	__syncthreads();
	for(int f=0; f<4; f++){
		d_output[threadIdx.x + f*(N/4) + blockIdx.x*N]=s_input[threadIdx.x + f*(N/4)];
	}
}
...
__global__ void FFT_GPU_multiple(float2 *d_input, float2* d_output, int N, int bits) {
	...
	s_input[threadIdx.x + blockDim.x]=d_input[threadIdx.x + blockDim.x + blockIdx.x*N];
	
	s_input[threadIdx.x + N/2]=d_input[threadIdx.x + blockDim.x + N/2 + blockIdx.x*N];
	s_input[threadIdx.x + blockDim.x + N/2]=d_input[threadIdx.x + blockDim.x + N/2 + blockIdx.x*N];
	...
	__syncthreads();
	d_output[threadIdx.x + blockIdx.x*N]=s_input[threadIdx.x + blockDim.x];
	d_output[threadIdx.x + blockDim.x + blockIdx.x*N]=s_input[threadIdx.x + blockDim.x];
	
	d_output[threadIdx.x + N/2 + blockIdx.x*N]=s_input[threadIdx.x + blockDim.x + N/2];
	d_output[threadIdx.x + blockDim.x + N/2 + blockIdx.x*N]=s_input[threadIdx.x + blockDim.x + N/2];
}
